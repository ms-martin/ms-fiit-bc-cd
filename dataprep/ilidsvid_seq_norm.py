...
def zero_mean_normalization(image, channels):
    image_channels = np.swapaxes(np.reshape(image, (-1, channels)), 0, 1)
    mean = np.mean(image_channels, 1)
    var = np.std(image_channels, 1)

    normalized = np.zeros((channels, np.shape(image_channels)[1]), np.float32)

    for i in range(channels):
        normalized[i] = (image_channels[i] - mean[i]) / var[i]

    return np.reshape(normalized, (-1))
...
def get_positive_pair(training):
    ...
    image1 = zero_mean_normalization(image1, 3)
    image2 = misc.imread(name="./i-LIDS-VID/sequences/cam2/" + p1 + "/" + img2)
    image2 = zero_mean_normalization(image2, 3)
