...
class Siamese:
    ...
    def convnet(self, inputx, training):
        ...
        conv2 = self.conv_layer(max1, [5, 5, 64, 128], [128], "conv2")
        ...
        max2flat = tf.reshape(max2, [self.batch_size, self.seq_len, 32 * 16 * 128], name="max2flat")

        hidden_size = 512
        rnn = self.rnn_layers(max2flat, 1, hidden_size)
        rnn = tf.reshape(rnn, [self.batch_size, hidden_size * self.seq_len], name="rnn_flat")

        dense1 = self.fc_layer(rnn, 1024, "dense1")

        dropout1 = tf.nn.dropout(dense1, 0.3 if training else 1.0, name="dropout1")
