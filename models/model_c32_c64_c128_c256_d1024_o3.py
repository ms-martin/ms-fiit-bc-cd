...
class Siamese:
    ...
    def convnet(self, inputx, training):
        ...
        conv4 = self.conv_layer(max3, [5, 5, 128, 256], [256], "conv4")

        max4 = tf.layers.max_pooling2d(inputs=conv4,
                                       pool_size=[2, 2],
                                       strides=2,
                                       name="max4")

        max4flat = tf.reshape(max4, [-1, 8 * 4 * 256], name="max4flat")

        dense1 = self.fc_layer(max4flat, 1024, "dense1")

        dropout1 = tf.nn.dropout(x=dense1,
                                 keep_prob=0.75 if training else 1.0,
                                 name="dropout1")
