...
class Siamese:
    ...
    def convnet(self, inputx, training):
        ...
        dense1 = self.fc_layer(max2flat, 4096, "dense1", True)
        ...
        out = self.fc_layer(dropout1, 128, "dense2", False)
        return out
    ...
    def fc_layer(self, _input, units, name, activation):
        ...
        biases = tf.get_variable(name=name + '_biases',
                                 ...
                                 initializer=tf.constant_initializer)
        if activation:
            return tf.nn.tanh(tf.nn.bias_add(tf.matmul(_input, weights), biases))
        else:
